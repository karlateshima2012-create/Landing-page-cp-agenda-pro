...
const FeatureSection = ({ title, description, items, imageUrl }: {
    ...
    <section className="py-12 md:py-24 bg-slate-950">
        <div className="max-w-[1440px] mx-auto relative z-10 px-6 md:px-16 lg:px-24">
            <div className="relative w-full bg-slate-900/40 border-2 border-brand-pink/60 rounded-[2rem] md:rounded-[4rem] py-16 md:py-24 px-8 md:px-16 lg:px-24 backdrop-blur-3xl shadow-[0_80px_160px_-40px_rgba(0,0,0,0.95)] overflow-hidden group">
                {/* Visual Effects */}
                <div className="absolute inset-0 bg-gradient-to-br from-brand-pink/10 to-transparent opacity-50 pointer-events-none"></div>
                <div className="absolute top-0 right-0 w-96 h-96 bg-brand-pink/10 blur-[100px] -z-10 rounded-full"></div>
                    ...
                    <div className="flex-1 text-center md:text-left">
                        <h2 className="text-3xl md:text-5xl font-black text-white mb-8 leading-tight tracking-tighter">
                            {title.split(" ").map((word, i) => (
                                <span key={i} className={word.toUpperCase() === "ONLINE" ? "text-brand-pink text-glow-pink" : ""}>
                                    {word}{" "}
                                </span>
                            ))}
                        </h2>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {items && items.map((item, idx) => (
                                <div key={idx} className="flex items-start gap-4 text-brand-gray group/item">
                                    <div className="w-6 h-6 rounded-full bg-brand-pink/10 flex items-center justify-center flex-shrink-0 mt-0.5 border border-brand-pink/20 group-hover/item:border-brand-pink/40 transition-colors">
                                        <Check className="w-3.5 h-3.5 text-brand-pink shadow-[0_0_10px_rgba(229,21,122,0.5)]" />
                                    </div>
